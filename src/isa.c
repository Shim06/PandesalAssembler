...
AddressingMode get_addressing_mode(Token* tokens, int start, int count)
{
    ...
    // Absolute addressing
    if (op1.type == TOKEN_ADDRESS && op2.type == TOKEN_COMMA && op3.text == "X") return ADDR_ABSOLUTEX;
    if (op1.type == TOKEN_ADDRESS && op2.type == TOKEN_COMMA && op3.text == "Y") return ADDR_ABSOLUTEY;
    if (op1.type == TOKEN_ADDRESS) return ADDR_ABSOLUTE;
    if (op1.type == TOKEN_IDENTIFIER) return ADDR_ABSOLUTE;
