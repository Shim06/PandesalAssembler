#include "isa.h"

const InstructionMnemonic instruction_mnemonic_map[MNEMONIC_COUNT] = {
    {"NOP", MNEM_NOP}, {"LDA", MNEM_LDA}, {"STA", MNEM_STA}, {"JMP", MNEM_JMP}, {"ADC", MNEM_ADC},
    {"AND", MNEM_AND}, {"ORA", MNEM_ORA}, {"XOR", MNEM_XOR}, {"NOT", MNEM_NOT}, {"LDX", MNEM_LDX},
    {"LDY", MNEM_LDY}, {"TAX", MNEM_TAX}, {"TAY", MNEM_TAY}, {"STX", MNEM_STX}, {"STY", MNEM_STY},
    {"TSX", MNEM_TSX}, {"TXA", MNEM_TXA}, {"TXS", MNEM_TXS}, {"TYA", MNEM_TYA}, {"SBC", MNEM_SBC},
    {"PHA", MNEM_PHA}, {"PHC", MNEM_PHC}, {"CLC", MNEM_CLC}, {"HLT", MNEM_HLT}, {"RTS", MNEM_RTS},
    {"CLV", MNEM_CLV}, {"BCC", MNEM_BCC}, {"BCS", MNEM_BCS}, {"BEQ", MNEM_BEQ}, {"BMI", MNEM_BMI},
    {"BNE", MNEM_BNE}, {"BPL", MNEM_BPL}, {"BVC", MNEM_BVC}, {"BVS", MNEM_BVS}, {"INX", MNEM_INX},
    {"INY", MNEM_INY}, {"SEC", MNEM_SEC}, {"DEX", MNEM_DEX}, {"DEY", MNEM_DEY}, {"CMP", MNEM_CMP},
    {"PLA", MNEM_PLA}, {"ASL", MNEM_ASL}, {"LSR", MNEM_LSR}, {"ROL", MNEM_ROL}, {"ROR", MNEM_ROR},
    {"CPX", MNEM_CPX}, {"CPY", MNEM_CPY}, {"INC", MNEM_INC}, {"ADD", MNEM_ADD}, {"SUB", MNEM_SUB},
    {"LD0", MNEM_LD0}, {"LD1", MNEM_LD1}, {"LD2", MNEM_LD2}, {"LD3", MNEM_LD3}, {"LD4", MNEM_LD4},
    {"TA0", MNEM_TA0}, {"TA1", MNEM_TA1}, {"TA2", MNEM_TA2}, {"TA3", MNEM_TA3}, {"TA4", MNEM_TA4},
    {"T0A", MNEM_T0A}, {"T1A", MNEM_T1A}, {"T2A", MNEM_T2A}, {"T3A", MNEM_T3A}, {"T4A", MNEM_T4A},
    {"TX0", MNEM_TX0}, {"TX1", MNEM_TX1}, {"TX2", MNEM_TX2}, {"TX3", MNEM_TX3}, {"TX4", MNEM_TX4},
};  

const uint16_t opcode_table[MNEMONIC_COUNT][ADDR_COUNT] = {
    [MNEM_NOP] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = NOP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LDA] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LDA_IMM,
        [ADDR_ABSOLUTE] = LDA_ABS,
        [ADDR_ABSOLUTEX] = LDA_ABSX,
        [ADDR_ABSOLUTEY] = LDA_ABSY
    },
    [MNEM_STA] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = STA_ABS,
        [ADDR_ABSOLUTEX] = STA_ABSX,
        [ADDR_ABSOLUTEY] = STA_ABSY,
        [ADDR_INDIRECT] = STA_IND
    },
    [MNEM_JMP] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = JMP_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_ADC] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = ADC_IMM,
        [ADDR_ABSOLUTE] = ADC_ABS,
        [ADDR_ABSOLUTEX] = ADC_ABSX,
        [ADDR_ABSOLUTEY] = ADC_ABSY
    },
    [MNEM_AND] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = AND_IMM,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = AND_ABSX,
        [ADDR_ABSOLUTEY] = AND_ABSY
    },
    [MNEM_ORA] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = ORA_IMM,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_XOR] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = XOR_IMM,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_NOT] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = NOT_IMM,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LDX] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LDX_IMM,
        [ADDR_ABSOLUTE] = LDX_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LDY] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LDY_IMM,
        [ADDR_ABSOLUTE] = LDY_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TAX] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TAX_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TAY] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TAY_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_STX] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = STX_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_STY] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = STY_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TSX] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TSX_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TXA] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TXA_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TXS] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TXS_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TYA] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TYA_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_SBC] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = SBC_IMM,
        [ADDR_ABSOLUTE] = SBC_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_PHA] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = PHA_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_PLA] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = PLA_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_PHC] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = PHC_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_CLC] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = CLC_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_HLT] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = HLT,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_RTS] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = RTS_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_CLV] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = CLV_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BCC] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BCC_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BCS] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BCS_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BEQ] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BEQ_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BMI] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BMI_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BNE] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BNE_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BPL] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BPL_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BVC] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BVC_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_BVS] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = BVS_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_INX] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INX_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_INY] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INY_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_SEC] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = SEC_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_DEX] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = DEX_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_DEY] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = DEY_IMP,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_CMP] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = CMP_IMM,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_ASL] =
    {
        [ADDR_ACCUMULATOR] = ASL_A,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = ASL_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LSR] =
    {
        [ADDR_ACCUMULATOR] = LSR_A,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = LSR_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_ROL] =
    {
        [ADDR_ACCUMULATOR] = ROL_A,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = ROL_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_ROR] =
    {
        [ADDR_ACCUMULATOR] = ROR_A,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = ROR_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_CPX] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = CPX_IMP,
        [ADDR_ABSOLUTE] = CPX_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_CPY] = 
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = CPY_IMP,
        [ADDR_ABSOLUTE] = CPY_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_INC] =
    {
        [ADDR_ACCUMULATOR] = INC_A,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INC_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_ADD] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = ADD_IMM,
        [ADDR_ABSOLUTE] = ADD_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_SUB] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = SUB_IMM,
        [ADDR_ABSOLUTE] = SUB_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LD0] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LD0_IMM,
        [ADDR_ABSOLUTE] = LD0_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LD1] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LD1_IMM,
        [ADDR_ABSOLUTE] = LD1_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LD2] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LD2_IMM,
        [ADDR_ABSOLUTE] = LD2_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LD3] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LD3_IMM,
        [ADDR_ABSOLUTE] = LD3_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_LD4] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = INVALID_OPCODE,
        [ADDR_IMMEDIATE] = LD4_IMM,
        [ADDR_ABSOLUTE] = LD4_ABS,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TA0] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TA0_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TA1] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TA1_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TA2] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TA2_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TA3] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TA3_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TA4] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TA4_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_T0A] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = T0A_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_T1A] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = T1A_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_T2A] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = T2A_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_T3A] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = T3A_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_T4A] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = T4A_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TX0] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TX0_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TX1] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TX1_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TX2] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TX2_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TX3] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TX3_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    },
    [MNEM_TX4] =
    {
        [ADDR_ACCUMULATOR] = INVALID_OPCODE,
        [ADDR_IMPLIED] = TX4_IMPL,
        [ADDR_IMMEDIATE] = INVALID_OPCODE,
        [ADDR_ABSOLUTE] = INVALID_OPCODE,
        [ADDR_ABSOLUTEX] = INVALID_OPCODE,
        [ADDR_ABSOLUTEY] = INVALID_OPCODE
    }
    
    //[ADDR_ACCUMULATOR] = INVALID_OPCODE,
    //[ADDR_IMPLIED] = INVALID_OPCODE,
    //[ADDR_IMMEDIATE] = INVALID_OPCODE,
    //[ADDR_ABSOLUTE] = INVALID_OPCODE,
    //[ADDR_ABSOLUTEX] = INVALID_OPCODE,
    //[ADDR_ABSOLUTEY] = INVALID_OPCODE
};


uint8_t get_instruction_size(AddressingMode mode)
{
    switch (mode)
    {
    case ADDR_ACCUMULATOR:
    case ADDR_IMPLIED:
        return 1;
    case ADDR_IMMEDIATE:
    case ADDR_ZEROPAGE:
    case ADDR_ZEROPAGEX:
    case ADDR_ZEROPAGEY:
    case ADDR_INDIRECTX:
    case ADDR_INDIRECTY:
    case ADDR_RELATIVE:
        return 2;
    case ADDR_ABSOLUTE:
    case ADDR_ABSOLUTEX:
    case ADDR_ABSOLUTEY:
    case ADDR_INDIRECT:
        return 3;
    }

    return 0;
}

AddressingMode get_addressing_mode(Token* tokens, int start, int count)
{
    //printf("start: %i, count: %i\n", start, count);
    if (start == count) return ADDR_IMPLIED;

    Token op1 = tokens[start];
    Token op2 = (start + 1 < count) ? tokens[start + 1] : (Token){ .type = TOKEN_UNKNOWN };
    Token op3 = (start + 2 < count) ? tokens[start + 2] : (Token){ .type = TOKEN_UNKNOWN };

    if (op1.type == TOKEN_IMMEDIATE) return ADDR_IMMEDIATE;
    if (op1.type == TOKEN_REGISTER && op1.text[0] == 'A') return ADDR_ACCUMULATOR;

    // Absolute addressing
    if (op1.type == TOKEN_ADDRESS || op1.type == TOKEN_IDENTIFIER)
    {
        if (op2.type == TOKEN_COMMA && strcmp(op3.text, "X") == 0) return ADDR_ABSOLUTEX;
        if (op2.type == TOKEN_COMMA && strcmp(op3.text, "Y") == 0) return ADDR_ABSOLUTEY;
        return ADDR_ABSOLUTE;
    }

    // Todo: Indirect (X,Y), ZeroPage (X, Y), 
    if (op1.type == TOKEN_OPENPAREN)
    {
        if (op2.type == TOKEN_ADDRESS || op2.type == TOKEN_IDENTIFIER)
        {
            if (op3.type == TOKEN_CLOSEPAREN) return ADDR_INDIRECT;
        }
    }
    return -1;
}

uint16_t get_opcode(Mnemonic mnemonic, AddressingMode addressing_mode)
{
    return opcode_table[mnemonic][addressing_mode];
}

HashTable* initialize_mnemonic_table(void)
{
    HashTable* table = create_hash_table();
    if (!table) return NULL;
    size_t map_size = sizeof(instruction_mnemonic_map) / sizeof(instruction_mnemonic_map[0]);
    for (size_t i = 0, size = map_size; i < size; i++)
    {
        if (insert(table, instruction_mnemonic_map[i].str, instruction_mnemonic_map[i].mnemonic_enum) != 0)
            return NULL;
    }
    return table;
}

uint8_t is_register(const char* str)
{
    if (str[1] != '\0') return 0;

    switch (str[0])
    {
    case 'A':
    case 'a':
    case 'X':
    case 'x':
    case 'Y':
    case 'y':
        return 255;
    default: return 0;
    }
}
